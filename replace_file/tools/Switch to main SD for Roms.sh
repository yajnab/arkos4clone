...
#then
  ...
  sudo cp /usr/local/bin/Switch\ to\ SD2\ for\ Roms.sh /opt/system/Advanced/ && sudo chmod 0777 /opt/system/Advanced/Switch\ to\ SD2\ for\ Roms.sh && sudo chown ark:ark /opt/system/Advanced/Switch\ to\ SD2\ for\ Roms.sh
  sudo rm /opt/system/Advanced/Switch\ to\ main\ SD\ for\ Roms.sh
  sudo cp -f /etc/samba/smb.conf.orig /etc/samba/smb.conf
  sudo umount /roms2
  sudo cp "/etc/emulationstation/es_systems.cfg.single" "/etc/emulationstation/es_systems.cfg"
  echo "es_systems.cfg updated."
  sudo mv "/opt/system/Advanced/Read from SD1 and SD2 for Roms.sh" "/opt/system/Advanced/Read from SD1 and SD2 for Roms"
  echo "Read from SD1 and SD2 Script Deactivated."
  sleep 3
