...
if grep -q '<string name="Language" value="zh-CN" />' /home/ark/.emulationstation/es_settings.cfg; then
    SDL_GAMECONTROLLERCONFIG="$sdl_controllerconfig" LANG=zh_CN.UTF-8 LC_ALL=zh_CN.UTF-8 /opt/flycastsa-2022/flycast "$2"
else
    SDL_GAMECONTROLLERCONFIG="$sdl_controllerconfig" /opt/flycastsa-2022/flycast "$2"
fi
sudo systemctl stop killer_daemon.service
sudo systemctl restart ogage &
...
DEVICENAME="RGB10"
    ...
    case "$CONSOLE_VAL" in
        u8|r50s|dr28s|r40s|rgb10max1|rgb10)
            DEVICENAME="U8"
            ;;
        a10miniv4)
